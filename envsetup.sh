...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^asylum_") ; then
        ASYLUM_BUILD=$(echo -n $1 | sed -e 's/^asylum_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $ASYLUM_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        ASYLUM_BUILD=
    fi
    export ASYLUM_BUILD
...
function print_lunch_menu()
{
    ...
    if [ "z${ASYLUM_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    target=$1
    local variant=$2
    ASYLUM_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/asylum/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the ASYLUM model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch asylum_$target-$variant
        fi
    fi
    return $?
}
...
function repopick() {
    T=$(gettop)
    $T/vendor/asylum/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $ASYLUM_FIXUP_COMMON_OUT ]; then
...
function installboot()
{
    ...
    if (adb shell getprop ro.asylum.device | grep -q "$ASYLUM_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.asylum.device | grep -q "$ASYLUM_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
. $ANDROID_BUILD_TOP/vendor/asylum/build/envsetup.sh
